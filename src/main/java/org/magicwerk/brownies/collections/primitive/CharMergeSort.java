...
public class CharMergeSort {
    ...
    private CharMergeSort(ICharList list) {
        this.list = list;

    }

    public static <E> void sort(ICharList list) {
        CharMergeSort sort = new CharMergeSort(list);
        sort.sort();
    }
    ...
    private int compare(int idx1, int idx2) {
        char val1 = list.get(idx1);
        char val2 = list.get(idx2);
        return (val1 < val2 ? -1 : (val1 == val2 ? 0 : 1));
    }

    private void swap(int idx1, int idx2) {
        char val = list.get(idx1);
