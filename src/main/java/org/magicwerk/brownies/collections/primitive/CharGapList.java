...
public class CharGapList extends ICharList {
     ...
     */
    private static char[] EMPTY_VALUES = new char[0];

    // -- EMPTY --
    // Cannot make a static reference to the non-static type E:
    // public static CharGapList EMPTY = CharGapList.create().unmodifiableList();
    // Syntax error:
    // public static  CharGapList EMPTY = CharGapList.create().unmodifiableList();
    /**
...
void ensureNormalized(int minCapacity) {
    ...
    char[] newValues = (char[]) new char[newCapacity];
...
private void moveData(int src, int dst, int len) {
    ...
    // Write (char) 0 into array slots which are not used anymore
...
protected void doEnsureCapacity(int minCapacity) {
    ...
    if (size == 0) {
        ...
        newValues = (char[]) new char[newCapacity];
...
public int binarySearch(int index, int len, char key) {
    checkRange(index, len);
    ensureNormalized(size);
    return ArraysHelper.binarySearch((char[]) values, index, index + len, key);
}
...
private void debugCheck() {
    // If the CharGapList is not used for storing content in KeyListImpl, values may be (char) 0
    if (values == null) {
    ...
    // Check that gap positions contain (char) 0 values
    if (gapSize > 0) {
    ...
    // Check that all end positions contain (char) 0 values
    if (start < end) {
 ...
 * It is only used for debugging.
 *
 * @return	state in which CharGapList is
 */
